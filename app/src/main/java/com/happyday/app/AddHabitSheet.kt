...
class AddHabitSheet : BottomSheetDialogFragment() {
    ...
    private val moreFacts = listOf(
        ...
        "Doing something just 21 days rewires your brain permanently.",
        ...
        "Showing up consistently, even imperfectly, is the secret weapon.",
        "You don't rise to the level of your goals, you fall to your systems."
    )
    private val lessFacts = listOf(
        "Reducing a bad habit by even 20% significantly improves wellbeing.",
        "Awareness is the first step — tracking keeps you honest with yourself.",
        "Progress, not perfection. Every day you resist counts.",
    ...
    override fun onCreateView(i: LayoutInflater, c: ViewGroup?, s: Bundle?) =
        i.inflate(R.layout.sheet_add_habit, c, false)
    ...
    override fun onViewCreated(v: View, s: Bundle?) {
        ...
        val factCard = v.findViewById<View>(R.id.factCard)
        ...
        editing?.let { nameIn.setText(it.name); dir = it.goalDirection; period = it.trackingPeriod; count = it.goalCount }
        cntTv.text = count.toString()

        fun refreshDir() {
            val more = dir == "do_more"
            doMore.setBackgroundResource(if (more) R.drawable.toggle_on else R.drawable.toggle_off)
            doLess.setBackgroundResource(if (!more) R.drawable.toggle_on else R.drawable.toggle_off)
            doMore.setTextColor(if (more) 0xFFFFFFFF.toInt() else 0xFF1C1C1E.toInt())
            doLess.setTextColor(if (!more) 0xFFFFFFFF.toInt() else 0xFF1C1C1E.toInt())
            factTv.text = if (more) moreFacts.random() else lessFacts.random()
            factCard.visibility = View.VISIBLE
            goalLbl.text = if (more) "Goal per period" else "Maximum per period"
        }
        fun refreshPeriod() {
            listOf(daily to "daily", weekly to "weekly", monthly to "monthly").forEach { (b, p) ->
                b.setBackgroundResource(if (period == p) R.drawable.toggle_on else R.drawable.toggle_off)
                b.setTextColor(if (period == p) 0xFFFFFFFF.toInt() else 0xFF1C1C1E.toInt())
            }
        }
        ...
        saveBtn.setOnClickListener {
            ...
            if (n.isEmpty()) { nameIn.error = "Enter a name"; return@setOnClickListener }
            val today = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(Date())
            onSave?.invoke(Habit(
                id = editing?.id ?: UUID.randomUUID().toString(),
                name = n, goalDirection = dir, trackingPeriod = period,
                goalCount = count, startDate = editing?.startDate ?: today,
