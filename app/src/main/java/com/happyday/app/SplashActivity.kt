    ...
    override fun onCreate(s: Bundle?) {
        ...
        val tick  = object : Runnable {
            override fun run() {
                ...
                pb.progress = ((e.toFloat() / DURATION) * 1000).toInt().coerceAtMost(1000)
                if (e < DURATION) handler.postDelayed(this, 40)
            }
        }
        handler.post(tick)
        handler.postDelayed({ go() }, DURATION)
        findViewById<Button>(R.id.skipBtn).setOnClickListener { handler.removeCallbacksAndMessages(null); go() }
    }
