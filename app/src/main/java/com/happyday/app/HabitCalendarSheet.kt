...
class HabitCalendarSheet : BottomSheetDialogFragment() {
    ...
    override fun onCreateView(i: LayoutInflater, c: ViewGroup?, s: Bundle?) =
        i.inflate(R.layout.sheet_habit_calendar, c, false)
    ...
    override fun onViewCreated(v: View, s: Bundle?) {
        ...
        val wkSdf = SimpleDateFormat("MMM d", Locale.getDefault())

        v.findViewById<TextView>(R.id.calName).text = habit.name
        val logged = habit.completedDates.values.count { it >= habit.goalCount }
        val start  = if (habit.startDate.isNotEmpty())
            try { disp.format(sdf.parse(habit.startDate)!!) } catch (e: Exception) { "—" } else "—"
        v.findViewById<TextView>(R.id.calStats).text = "$logged days logged  •  started $start"
        v.findViewById<ImageButton>(R.id.calClose).setOnClickListener { dismiss() }

        buildGrid(v.findViewById(R.id.calGrid), habit, sdf, wkSdf)
    }

    private fun buildGrid(container: LinearLayout, habit: Habit, sdf: SimpleDateFormat, wkSdf: SimpleDateFormat) {
        ...
        fun hTv(t: String) = TextView(ctx).apply {
            text = t; textSize = 10f; gravity = Gravity.CENTER
            setTextColor(0xFF8E8E93.toInt())
            layoutParams = LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f)
        }
        hdr.addView(hTv("").apply { layoutParams = LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1.6f) })
        ...
        while (!cal.after(today)) {
            ...
            // Label
            val lbl = "${wkSdf.format(wkStart.time)}-${wkEnd.get(Calendar.DAY_OF_MONTH)}"
            row.addView(TextView(ctx).apply {
                text = lbl; textSize = 9f; gravity = Gravity.CENTER_VERTICAL
                setTextColor(0xFF8E8E93.toInt())
            ...
            for (i in 0 until 7) {
                ...
                val done   = !future && (habit.completedDates[sdf.format(dayCal.time)] ?: 0) >= habit.goalCount
                val size   = 32 * dp
                row.addView(TextView(ctx).apply {
                    ...
                    when {
                        future -> { /* blank */ }
                        done   -> { text = "✓"; setTextColor(0xFF27AE60.toInt()); setBackgroundResource(R.drawable.cal_done) }
