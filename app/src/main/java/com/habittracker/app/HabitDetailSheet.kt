...
class HabitDetailSheet : BottomSheetDialogFragment() {
    ...
    private val disp = SimpleDateFormat("MM/dd/yy",   Locale.getDefault())
    ...
    private val quotes = listOf(
        ...
        "The only bad habit is the one you never bothered to track.",
    ...
    ) = inflater.inflate(R.layout.sheet_habit_detail, container, false)
    ...
    override fun onViewCreated(v: View, savedInstanceState: Bundle?) {
        ...
        val cur = currentStreak(habit)
        val lng = longestStreak(habit)
        ...
        v.findViewById<TextView>(R.id.detailLogged).text =
        ...
        v.findViewById<ImageButton>(R.id.btnMarkDone).setOnClickListener {
            ...
            if (idx >= 0) {
                ...
                if (next == 0) h.completedDates.remove(today)
                else           h.completedDates[today] = next
                dm.saveHabits(list)
                refreshStatus(h)
            }
        }
    ...
    private fun currentStreak(h: Habit): Int {
        ...
        var s = 0
        while ((h.completedDates[sdf.format(cal.time)] ?: 0) >= h.goalCount) {
    ...
    private fun longestStreak(h: Habit): Int {
        ...
        val sorted = h.completedDates.entries
            .filter  { it.value >= h.goalCount }
            .mapNotNull { try { sdf.parse(it.key) } catch (e: Exception) { null } }
            .sortedBy { it.time }
        ...
        for (i in 1 until sorted.size) {
            ...
            if (sdf.format(cal.time) == sdf.format(sorted[i])) {
                cur++; if (cur > lng) lng = cur
            } else cur = 1
        }
        return lng
    }
