...
class HabitAdapter(
    ...
    inner class VH(v: View) : RecyclerView.ViewHolder(v) {
        ...
        val stat: TextView           = v.findViewById(R.id.habitStats)
    ...
    override fun onBindViewHolder(h: VH, pos: Int) {
        ...
        val cnt   = habit.completedDates[today] ?: 0
        val done  = cnt >= habit.goalCount

        h.name.text = habit.name

        if (habit.goalDirection == "do_less") {
            h.box.text = cnt.toString()
            h.box.setBackgroundResource(
                if (cnt >= habit.goalCount) R.drawable.cb_red else R.drawable.cb_outline_red
            )
            h.box.setTextColor(0xFFE74C3C.toInt())
        } else {
        ...
        h.stat.text = parts.joinToString("   ")
        ...
        h.box.setOnClickListener {
            ...
            if (next == 0) habit.completedDates.remove(today)
            else           habit.completedDates[today] = next
            onSave()
    ...
    private fun calcStreak(habit: Habit): Int {
        val cal = Calendar.getInstance()
